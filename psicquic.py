...
class MitabTopology(object):
    ...
    def __init__OLD(self, psqObject):
        ...
        for psqData in psqObject:
            ...
            y = psqData.interactors[1][0][1]
            
            for _self in (x, y):
                _other = y if _self == x else x
                #print ("self " + _self + " // other " + _other)
                if not _self in self.tmpAdj:
                    self.tmpAdj[_self] = {}
                if not _other in self.tmpAdj[_self]:
                    self.tmpAdj[_self][_other] = []
                self.tmpAdj[_self][_other].append( psqData )
                if x == y:
                    break


        wRank = sorted( [ (x, len( self.tmpAdj[x].keys() )) for x in self.tmpAdj ], key= lambda o : o[1], reverse=True)
      #  print (wRank)
        registred = set()
        miSet = set()
       # c = 0
           ...
           # print(x, xCount)
            ...
            for y in self.tmpAdj[x]:
                if y in registred and x != y:
                    continue
                partnerData[y] = self.tmpAdj[x][y]             
                #c += len(partnerData[y])

            if partnerData:
                #print (partnerData)
                for _x in partnerData:
                    for _y in partnerData[_x]:
                        if _y in miSet:
                            raise ValueError(_y)
                        miSet.add(_y)
    ...
    def get(self, k1, k2):
        if k1 in self.reduxAdj:
            if k2 in self.reduxAdj[k1]:
                return self.reduxAdj[k1][k2]

        if k2 in self.reduxAdj:
            if k1 in self.reduxAdj[k2]:
                return self.reduxAdj[k2][k1]
